...
sep_stmt  : assignment {}
          ...
          | EVAL '(' expr ')' {auto str = $3->trueValueStr(); std::cout << "\033[1;32mEVAL: [\033[0m" << str << "\033[1;32m]\033[0m\n";}
          | TYPEOF '(' expr ')' {auto str =  $3->typeStr(); std::cout << "\033[1;36mTYPEOF: [\033[0m" << str << "\033[1;36m]\033[0m\n";}
...
non_sep_stmt   : if_expr ELSE ':' block {} 
               ...
               | FOR {symbolTable.enterAnonymousScope();} assignment {} SEP expr { if ($6->type() != TypeNms::BOOL) {yyerror("Condition of for loop has to be of boolean type");}} SEP assignment ':'  '{' statement_list '}' {symbolTable.exitScope();}
               | FOR {symbolTable.enterAnonymousScope();} decl_assign {} SEP expr { if ($6->type() != TypeNms::BOOL) {yyerror("Condition of for loop has to be of boolean type");}} SEP assignment ':'  '{' statement_list '}' {symbolTable.exitScope();}
               | block {}
               ;
